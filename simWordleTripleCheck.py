...
def AIchoice(wordslist,result,word):
    ...
    for x in right:
        ...
        for y in range(len(result)):
            if x[y]!=result[y] and result[y]!="." and result[y]!="?":
                bad=True
            if x[y]==result[y]:
                good=True
                
        if good==True and bad==False:
            # Performs additional check for "?" placements
            ...
            for y in range(len(result)):
                if result[y] == "?":
                    if x[y]==word[y]:
                        bad=True
            if bad==False:
                trippleRight.append(x)

    if len(doubleRight)==0:
        doubleRight=right.copy()
    if len(trippleRight)==0:
        trippleRight=doubleRight.copy()
    
    return trippleRight


wordlists=setup()
#PlayGame(8,wordlists.copy())
start1=datetime.now()
